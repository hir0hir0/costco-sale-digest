...
## レシート取り込み（買った値段を基準にする）

`costco_data/purchases.json` にレシートの明細を入れると:

- 明細の価格が「店頭の実売価格」として価格履歴に入る（メルマガに載らない定番品も追える）
- セール価格が**自分が前回買った実質額（クーポン込み）より安い**とき、
  サイトに「前回購入より安い」バッジが付く

取り込み方: レシートを写真に撮って Claude のセッションに貼り、「レシート取り込んで」
と頼む（Claude が読み取って purchases.json に追記・コミットする）。手で書いてもよい:

```json
{"month": "2026-01",
 "items": [{"item_no": "1234567", "name": "サンプル商品 3P", "price": 1980, "coupon": 300},
           {"item_no": "7654321", "name": "サンプル精肉", "price": 2480, "weight_g": 620}]}
```

購入店舗・購入日（日単位）・レシート合計は保存しない。量り売りの商品は `weight_g`
または `unit_price`（¥/100g）を入れる — 総額は重さで変わるので、無いと価格の比較が
成り立たない。同じ月に同じ商品が複数行あるときは1行に潰される（買った数量は出さない）。

⚠️ このファイルは公開リポジトリに入る＝**買った物と購入月が公開される**。
嫌になったらファイルを空配列 `[]` にすれば、次の更新で履歴からも消える
（履歴は毎回レシートから再構成ではなく追記なので、正確には backfill で作り直す）。
