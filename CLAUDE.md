...
## 他店チラシのメモ（ユーザーがチラシ画像を貼って「チラシ取り込んで/メモして」と言ってくる）

コストコとの価格比較用に、近所のスーパー等のチラシを `costco_data/flyers.json` に
書き起こして蓄積する（初回: 三和 ららぽーと豊洲店の開店セール 2026-09-11〜14）。

```json
[{"store": "三和 ららぽーと豊洲店", "period": "2026-09-11/2026-09-14",
  "kind": "開店セール", "note": "…",
  "items": [{"genre": "精肉", "name": "国産黒毛和牛切落し 500g入",
             "price": 1580, "tax": 1707, "unit": "316円/100g", "days": "9/11"}]}]
```

- チラシ1枚（1企画）＝配列の1要素。**追記であって上書きしない**（過去チラシも比較材料）
- 他店の**レシート**も同じ配列に入れてよい: `kind:"レシート実売"`, `period:"YYYY-MM"`（月まで）。
  税抜価格のみ・購入日/数量/合計は書かない（コストコの purchases.json と同じ公開粒度）。
  取り込み時は Σ明細 = 小計 の検算を必ずする
- `price` は本体価格・`tax` は税込（チラシに併記があるときだけ）。値引率のみの行は
  `discount`（"半額"/"3割引" 等）。`days` は「その日限り」の日付。判読に自信が無い行は
  `note` に "判読注意" と書く（嘘の断定をしない）
- `genre` は精肉/鮮魚/惣菜/青果/日配/冷凍・菓子/グロサリー/米/酒/日用品 あたりで統一
- **このファイルはサイトには出ない**（build.py は読まない）。用途はセッション内での
  コストコとの比較・相談。サイトに出したくなったら別途相談
- チラシの自動収集はできない（Actions からチラシサイトを巡回する仕組みは無く、
  セッションの egress も外部Webを遮断）。**ユーザーが画像を貼るのが取り込み経路**
